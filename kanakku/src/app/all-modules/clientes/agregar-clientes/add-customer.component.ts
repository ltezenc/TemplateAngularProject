...
export class AddCustomerComponent implements OnInit {
  ...
  crearCliente(): void {
    ...
    if (tarifa.value == "" && potenciaconcentrada.value == "" && tipocambio.value == "" && precioporhora.value == "" && preciofuerahorapunta.value == "" && estado.value == "" && ppi.value == "" && pgn.value == "" && comercializacion.value == "" && cargofijo.value == "") {
      ...
      this.clienteService.create(this.cliente).subscribe(data => {
        this.estregistro = true;
        alertifyjs.success('¡Cliente nuevo registrado!');
        this.router.navigate(['/clientes/listar-clientes'])
      },error => alertifyjs.error('¡Ocurrió un '+error.status+' error al registrar!'))
    }
  }
  verificarNombre($n) {
  ...
  nextTab() {
    ...
    if (razonsocial.value == "" && ruc.value == "" && direccion.value == "" && responsablelegal.value == "" && email.value == "" && telefono.value == "") {
      ...
      (!this.verificarNumCel(ruc.value) || !this.verificarNumCel(telefono.value) || !this.verificarCorreo(email.value))
    ) {
